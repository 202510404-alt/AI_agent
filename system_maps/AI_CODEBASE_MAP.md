...
├── tools/
│   ├── python_agent_tools/
│   │   ├── agent_navigator.py [📂 tools/python_agent_tools/agent_navigator.py] -> [💡 📦 imp: json, pathlib.Path, re, sys, tkinter, tkinter.filedialog | 🧬 class SemanticNavigator [L11-188] |    └─ def __init__(root_dir) [L12-16] |    └─ def _load_database() [L18-25] |    └─ def extract_multi_slices(raw_prompt) [L30-188] | 🧬 class JjapCursorNavigatorGUI [L193-314] |    └─ def __init__(root, project_root) [L194-243] |    └─ def execute_slicing_pipeline() [L245-295] |    └─ def manual_export_file() [L297-314]]
...
│   │   ├── indexer.py [📂 tools/python_agent_tools/indexer.py] -> [💡 📦 imp: ast, hashlib, json, os, pathlib.Path, typing.Any | 🧬 class AdvancedIndexerV2 [L25-208] |    └─ def __init__(project_root) [L34-42] |    └─ def _get_sha256(content) [L44-45] |    └─ def _extract_skeleton(content) [L47-63] |    └─ def parse_protocols_and_registries(content, rel_path_str) [L65-98] |    └─ def index_file(file_path) [L100-139] |    └─ def scan_project() [L141-208]]
